...
def check_tensorflow_version():
    min_tf_version = "1.4.0-dev20171024".split('.')
    version = tf.__version__.split('.')
    if float(version[0]) < float(min_tf_version[0]) or (
            float(version[0]) == float(min_tf_version[0]) and
            float(version[1]) < float(min_tf_version[1])):
...
def debug_tensor(s, msg=None, summarize=10):
    """Print the shape and value of a tensor at test time. Return a new
    tensor."""
    if not msg:
        msg = s.name
...
def add_summary(summary_writer, global_step, tag, value):
    ...
    Useful to log things that are not part of the training graph, e.g.,
    tag=BLEU.
    """
...
def get_config_proto(hparams=None, log_device_placement=False,
...
def format_bpe_text(symbols, delimiter=b"@@"):
    ...
    for symbol in symbols:
        if len(symbol) >= delimiter_len and symbol[
                                            -delimiter_len:] == delimiter:
