...
class GNMTModel(attention_model.AttentionModel):
    ...
    def _build_encoder(self, hparams):
        ...
        source_speaker = iterator.source_speaker
        target_speaker = iterator.target_speaker
        if self.time_major:
            source = tf.transpose(source)
        source_speaker = tf.transpose(source_speaker)
        target_speaker = tf.transpose(target_speaker)
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            encoder_emb_inp = tf.nn.embedding_lookup(self.embedding_encoder,
                                                     source)
            encoder_emb_spk1 = tf.nn.embedding_lookup(self.embedding_speaker,
                                                      source_speaker)
            encoder_emb_spk2 = tf.nn.embedding_lookup(self.embedding_speaker,
                                                      target_speaker)
            concat_spk = tf.concat((encoder_emb_spk1, encoder_emb_spk2), axis=1)

            # Execute _build_bidirectional_rnn from Model class
            bi_encoder_outputs, bi_encoder_state = \
            ...
            if self.time_major:
                dim = tf.shape(encoder_outputs)[0]
                tiled_speakers = tf.expand_dims(concat_spk, axis=0)
                tiled_speakers = tf.tile(tiled_speakers, [dim, 1, 1])
            else:
                dim = tf.shape(encoder_outputs)[1]
                tiled_speakers = tf.expand_dims(concat_spk, axis=1)
                tiled_speakers = tf.tile(tiled_speakers, [1, dim, 1])
            enhanced = tf.concat((encoder_outputs, tiled_speakers), axis=2)

            # Pass all encoder state except the first bi-directional layer's
            # state to decoder.
            encoder_state = (bi_encoder_state[1],) + (
                (encoder_state,) if num_uni_layers == 1 else encoder_state)

        return enhanced, encoder_state
    ...
    def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
        ...
        #TODO check correctness for number of units
        num_units = hparams.num_units
...
class GNMTAttentionMultiCell(tf.nn.rnn_cell.MultiRNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        """Run the cell with bottom layer's attention copied to all upper
        layers."""
        if not nest.is_sequence(state):
            raise ValueError(
                    "Expected state to be a tuple of length %d, but received: "
                    "%s"
        ...
        with tf.variable_scope(scope or "multi_rnn_cell"):
            ...
            with tf.variable_scope("cell_0_attention"):
                ...
                cur_inp, new_attention_state = attention_cell(inputs,
                                                              attention_state)
