...
def create_train_model(
    ...
    src_spk_file = "%s.%s" % (hparams.train_prefix, hparams.src)
    tgt_spk_file = "%s.%s" % (hparams.train_prefix, hparams.tgt)
    ...
    with graph.as_default(), tf.container(scope or "train"):
        ...
        src_spkr_dataset = tf.data.TextLineDataset(src_spk_file)
        tgt_spkr_dataset = tf.data.TextLineDataset(tgt_spk_file)
...
class EvalModel(
        collections.namedtuple("EvalModel",
                               ("graph", "model", "src_file_placeholder",
                                ...
                                "src_spk_file_placeholder",
                                "tgt_spk_file_placeholder", "iterator"))):
    pass
...
class InferModel(
        collections.namedtuple("InferModel",
                               ("graph", "model", "src_placeholder",
                                ...
                                "src_speaker_placeholder",
                                "tgt_speaker_placeholder"))):
    pass
...
class ProdModel(
        collections.namedtuple("ProdModel",
                               ("graph", "model", "src_placeholder",
                                "iterator",
                                "src_speaker_placeholder",
                                "tgt_speaker_placeholder"))):
...
def create_prod_model(model_creator, hparams, scope=None, extra_args=None):
    """Create inference model."""
    graph = tf.Graph()
    src_vocab_file = hparams.src_vocab_file
    tgt_vocab_file = hparams.tgt_vocab_file
    ...
    with graph.as_default(), tf.container(scope or "infer"):
        ...
        tgt_spkr_dataset = tf.data.Dataset.from_tensor_slices(
                tgt_spkr_placeholder)
        iterator = iterator_utils.get_prod_iterator(
                src_dataset,
                src_vocab_table,
                src_spkr_dataset,
                tgt_spkr_dataset,
...
def create_emb_for_encoder_and_decoder(share_vocab,
    ...
    if num_partitions <= 1:
        ...
        # embedding_lookup tries to colocate single partition-ed embedding
        # variable
        # with lookup ops. This may cause embedding variables being placed on
        #  worker
        # jobs.
    ...
    with tf.variable_scope(
            scope or "embeddings", dtype=dtype,
        ...
        if share_vocab:
            ...
            embedding_encoder = _create_or_load_embed(
                    name, vocab_file, embed_file,
                    src_vocab_size, src_embed_size, dtype)
            embedding_decoder = embedding_encoder
        else:
            with tf.variable_scope("encoder", partitioner=partitioner):
                embedding_encoder = _create_or_load_embed(
                        name + "_encoder", src_vocab_file, src_embed_file,
                        src_vocab_size, src_embed_size, dtype)

            with tf.variable_scope("decoder", partitioner=partitioner):
                embedding_decoder = _create_or_load_embed(
                        name + "_decoder", tgt_vocab_file, tgt_embed_file,
...
def avg_checkpoints(model_dir, num_last_checkpoints, global_step,
    ...
    if len(checkpoints) < num_last_checkpoints:
        utils.print_out(
                "# Skipping averaging checkpoints because not enough "
                "checkpoints is "
                "avaliable."
                )
        return None
    ...
    # Build a graph with same variables in the checkpoints, and save the
    # averaged
    # variables into the avg_model_dir.
    with tf.Graph().as_default():
        tf_vars = [
            tf.get_variable(v, shape=var_values[v].shape,
